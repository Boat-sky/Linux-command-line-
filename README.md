...
```
บางครั้งเราสั่งเลือก GPU ตอน run ด้วย `CUDA_VISIBLE_DEVICES=gpu_id` แต่ก็ยังมีบางส่วน run บน gpu 0 เพราะว่าใน code เดิมอาจใช้ `.cuda()` หรือ `.device('cuda:0')` ลองเช็คว่าไฟล์ไหนมีปัญหาด้วย
```
